...
export class ProductService {
  ...
  getProducts(): Observable<Product[]> {
    ...
    return this.http.get<Product[]>("http://localhost:8080/get-products");
  }

  buyProduct(product) {
    return this.http.post<{link: string}>("http://localhost:8080/payment", product.price);
  }

  deleteProduct(product) {
    return this.http.delete("http://localhost:8080/delete-product/" + product.id);
  }
}
